...
void update_H_master_top(Point space[], int Nx, int Ny, double mux[], double muy[], double dx, double dy,  Point row_bellow[], PML_coefs coefs_Hx[], PML_coefs coefs_Hy[], double ICHx[], double ICHy[])
{
    ...
    for (int i=0; i<Ny-1; ++i)
    {
        for (int j=0; j<Nx-1; ++j)
        {
            ...
            new_Hx = coefs_Hx[index(i,j,Nx)].Getm1() * space[index(i,j,Nx)].GetHx() 
            + coefs_Hx[index(i,j,Nx)].Getm2() * CEx
             + coefs_Hx[index(i,j,Nx)].Getm3() * ICHx[index(i,j,Nx)];
            
            
            new_Hy = coefs_Hy[index(i,j,Nx)].Getm1()* space[index(i,j,Nx)].GetHy() 
...
void update_E_master_top(Point space[], int Nx, int Ny, double ep[], double dx, double dy, double dt, PML_coefs coefs_Dz[], double IDz[])
{
    ...
    {
    int j{0};
    CHz = ((space[index(i,j,Nx)].GetHy()) / dx ) - ((space[index(i,j,Nx)].GetHx() - 0) / dy );
    
    IDz[index(i,j,Nx)] +=space[index(i,j,Nx)].GetDz();


    new_Dz = coefs_Dz[index(i,j,Nx)].Getm1() * space[index(i,j,Nx)].GetDz() 
    ...
    {
        CHz = ((space[index(i,j,Nx)].GetHy() - space[index(i,j,Nx)].GetHy()) / dx ) - ((space[index(i,j,Nx)].GetHx() - 0) / dy );
    ...
    for (int i=1; i<Ny; ++i)
    {
        ...
        //ylow only, i!=0, j=0
        int j{0};
        CHz = ((space[index(i,j,Nx)].GetHy()) / dx ) - ((space[index(i,j,Nx)].GetHx() - space[index(i,j,Nx)].GetHx()) / dy );
